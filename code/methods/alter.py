...
class Alter:
    ...
    def loop(self, t, epochs, train_loader, test_loader, pretrain=None, add_train=False):
        ...
        Args:
            t (int): The task/scenario number
            ...
            add_train (bool): Flag about whether the training procedure is additional knowledge or not
        """
    ...
    def test(self, t, test_loader):
        self.model.eval()
        with torch.no_grad():
            return self._iteration_test(t, test_loader, self.mse)
    ...
    def _epoch_postprocessing(self, t, nmse_list):
        if nmse_list is not None:
            if self.add_train:
                nmse = nmse_list[-1]    # this can be adjusted with different standards
            else:
                nmse = nmse_list[-1]
